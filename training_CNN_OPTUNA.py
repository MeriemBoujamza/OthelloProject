...
from networks_e2205028 import MLP,LSTMs,CNN_OPTUNA
...
def objective(trial):
    ...
    opti =trial.suggest_categorical("optimizer", ["Adam", "Adagrad"])
    acFun = trial.suggest_categorical("activationFunction", ["Tanh", "ReLU", "LeakyReLU"])
    learningRate =trial.suggest_float('learning_rate', 0.001, 0.003)
    ...
    model = CNN_OPTUNA(conf,acFun).to(device)    
...
study = optuna.create_study(study_name="CNN_Optimization", direction="maximize",storage="sqlite:///optunaCNNWinner.db",load_if_exists=True)
