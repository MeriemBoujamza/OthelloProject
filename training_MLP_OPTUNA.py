...
from networks_e2205028 import MLP_OPTUNA,LSTMs
...
# opt = torch.optim.Adamax(model.parameters(), lr=0.005) 
...
#                        device, opt)
    ...
    # nodes3 = trial.suggest_int('nodes3',100,200)
    # nodes_second_layer = trial.suggest_int('nodes2',300,400)

def objective(trial):
    # Define the hyperparameters to optimize
    dropoutRate = trial.suggest_float("dropout", 0.2, 0.4)    
    opti =trial.suggest_categorical("optimizer", ["Adam", "RMSprop", "Adagrad"])
    acFun = trial.suggest_categorical("activationFunction", ["Tanh", "LeakyReLU", "ReLU"])
    learningRate =trial.suggest_float('learning_rate', 0.001, 0.007)
    nodes_first_layer = trial.suggest_int('nodes1',400,600)
    nodes_second_layer = trial.suggest_int('nodes2',200,300)
    ...
    conf['nodesFirst']=nodes_first_layer
    conf['nodesSec']=nodes_second_layer
    conf['activationFunction']=acFun
    #build the model 
    model = MLP_OPTUNA(conf).to(device)    
    optimizer = getattr(optim, opti)(model.parameters(), lr= learningRate)
...
study = optuna.create_study(study_name="MLP_Optimization_SC", direction="maximize",storage="sqlite:///optunaMLPWinner.db")
